...
--
-- Truncado de posible contenido en tablas que puedan afectar al volcado posterior
--
...
ALTER TABLE login 
  ADD CONSTRAINT LO_LOTI_CK CHECK (tipo IN('A','U','D','a','u','d')
COMMIT;
...
ALTER TABLE partido
  ADD CONSTRAINT PA_PEQL_FK FOREIGN KEY (codLocal) REFERENCES equipo (codEquipo) ON DELETE CASCADE,
  ADD CONSTRAINT PA_PEQV_FK FOREIGN KEY (codVisitante) REFERENCES equipo (codEquipo) ON DELETE CASCADE,
  ADD CONSTRAINT PA_PAJO_FK FOREIGN KEY (codJornada) REFERENCES jornada (codJornada) ON DELETE CASCADE;

----------------------------------------------------------

--
-- INSERCCION PREVIA DE DATOS PARA LAS TABLAS ANTERIORMENTE VOLCADAS - ESTOS DATOS SON OBLIGATORIOS
-- POR LO TANTO SON DATOS DE CARACTER PREESTABLCIDO PARA EL FUNCIONAMIENTO INICIAL DEL PROGRAMA
--

----------------------------------------------------------

--
-- Datos para la tabla 'usuario' y 'login' -  Creacion del usuario SUPERAMINISTRADOR
--
